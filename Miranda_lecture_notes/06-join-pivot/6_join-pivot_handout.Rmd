...
left_join(x = discounts, y = drinks, by = join_by(Cafe))
...
In the original discounts and drinks data, both have a column named `Cafe`, which R can recognize.
...
right_join(discounts_rows, drinks_rows)
```
