---
title: "Section 13: Inference for a Single Mean"
...
Let's also focus on a single group: female finishers age 18-34, from 2010.

```{r}
bm_small <- bm %>% 
  filter(Sex == "female", Age_Range == "18-34", Year == 2010)

ggplot(bm_small, aes(x = Time))+
  geom_histogram() +
  ggtitle("2010 Boston Marathon Finishing Times for Females 18-34")
```
...
```{r}
# Create a fake "population" of finishing times
set.seed(4313)
pretend_pop <- c(rnorm(3000, 213, 15), rnorm(2000, 268, 35))

ggplot(mapping = aes(x = pretend_pop)) +
  geom_density() +
  labs(title = "Population of Finishing Times")
```
...
Our p-value is P(T <= -7.41) + P(T >= 7.41) on the T with n-1 degrees of freedom, found with `pt()`.
...
Consider female runners age 60-64 from 2010.  Do we have evidence that their average time is less than 280 minutes?  How can we check this with `t.test()`?
