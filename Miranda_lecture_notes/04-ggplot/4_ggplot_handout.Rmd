...
useful_values <- meanDuration +
  c(-3, -2, -1, 0, 1, 2, 3) * sd(mendota$duration)
...
ggplot(mendota, aes(duration)) +
  ...
  geom_vline(xintercept = useful_values,
             size = 1, linetype = c(2, 2, 2, 1, 2, 2, 2)) 
```
...
Returning to the `geom_point()` scatterplot, let's add the number of intervals in our mapping.
...
In the following plot, the aesthetic of `intervals` applies to both the lines and the dots.  Since we're treating `intervals` as categorical, the lines are connected incorrectly.
...
ggplot(mendota, aes(x = year1, y = duration) ) + 
  geom_line() + 
  geom_point(mapping = aes(color = as.factor(intervals))) 
```
...
```{r}
ggplot(mendota, aes(x = year1, y = duration, color = duration)) +
  geom_point() +
...
Example: How well do the Lake Mendota freeze durations match a theoretical normal bell-curve?

```{r}
# Get mean and sd to draw normal curve
duration_mean <- mean(mendota$duration)
duration_sd <- sd(mendota$duration)
...
ggplot(mendota, aes(duration)) +
  ...
  geom_function(
  fun = dnorm,
  args = list(mean = duration_mean, sd = duration_sd),
  n = 1001
)
```
