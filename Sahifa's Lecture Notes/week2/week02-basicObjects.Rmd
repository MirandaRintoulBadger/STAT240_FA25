...
* This file should be in `STAT240/lecture/week02`.
...
> You can create a new variable with the expression `newVar = ...`, where `...` is the information you want the variable to store.

* You may use `<-` or `=`; I was taught to use `=` but many others were taught to use `<-`. For all intents and purposes, they are the same.

* If you want to edit the contents of an existing variable, you use that exact same code, `newVar = ...`, to overwrite its existing contents.
...
* Now, in expressions which use `myFavNum`, 12 will be substituted wherever you put that name.

```{r}
myFavNum + 2
```

* Notice that last code chunk does NOT have the `newVar = ...` form. The expression evaluates to 14, but this is not captured anywhere, so it just becomes output. The value of `myFavNum` is still 12.

```{r}
# The previous chunk did not STORE the output "myFavNum + 2" anywhere, so it gets "dumped" out to the console.

# The value of myFavNum has not changed.
...
# Two different ways to compute myFavNum to the power of 2
myFavNum**2
myFavNum^2
```
...
```{r}
# is myFavNum greater than 10 AND myFavNum + 2 less than 13?
# one of these fails, so the whole "and" condition fails.
(myFavNum > 10) & (myFavNum + 2 < 13)

# is myFavNum greater than 10 OR myFavNum + 2 less than 13?
...
```{r}
# The INCORRECT way to check if the value of myFavNum is equal to 13. This is replacing its value with 13, not checking if it is equal. Notice that it did not print a TRUE or FALSE, like you would expect if you correctly checked for equality!
myFavNumr = 13
...
# Creating a new variable called "something", which takes on the value of whatever myFavNum is.
# This is a totally distinct copy that has nothing to do with myFavNum going forward, they do not reference each other.
something = myFavNum
...
15:myFavNum # Extended use: backward sequences, and variable use. Remember how we changed myFavNum to 13 in the Variables section?
```
