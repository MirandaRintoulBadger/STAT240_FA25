...
<span style="color:#5858d0"> Orbital Brightness Modulation, Radial Velocity, Transit, and Transit Timing Variations are all able to detect small planets.  Given that some methods have very small sample size, it is difficult to tell how similar the methods are.  It seems like Imaging struggles to detect a planet with radius less than 10.  Out of all of the methods, Transit detected the planet with the smallest radius. </span>
...
```{r, class.source="soln", class.output="soln"}
ggplot(methods_within_year_proportions,
       aes(year, methodProportion, fill = method)) +
  geom_col()
```

Finally, let's answer the client's question. How has the most popular method of planet discovery changed over time?

<span style="color:#5858d0"> Around 2000, Radial Velocity was the most popular method, but Transit began to grow in popularity and took over as the most popular method around 2010. </span>
