...
<span style="color:#5858d0"> The player would need to independently hit the 5/6 chance of not rolling a 1 eight times in a row.  So the probability is $\Big(\frac{5}{6}\Big)^8 = 0.233.$ </span>

(b) A player has successfully managed to roll 7 times.  What is the probability that they "greed out" on the eigth roll?

<span style="color:#5858d0"> The probability on the eigth roll is not affected by the results of the previous rolls, so it is still just 1/6. </span>
...
<span style="color:#5858d0"> If Tammy rolls a 4, 5, or 6 on her first roll, which happens with probability 1/2, she beats Tony immediately. If she rolls a 2 or 3 for her first roll (probability 1/3), she doesn't beat Tony, but anything other than a 1 on her second roll (probability 5/6) will end up beating Tony.  So, the total probability is 
$$\Big(\frac{1}{2}\Big) \;+\; \Big(\frac{1}{3}\Big)\cdot\Big(\frac{5}{6}\Big) = 0.778$$
</span>
...
<span style="color:#5858d0"> $X$ is discrete since it can only take on specific values. </span>

(b) What are all of the possible values of $X$?

<span style="color:#5858d0"> $X$ will be 0 if Gretchen "greeds out". The minimum score she gets if she manages to roll successfully is 2(8) = 16.  Any whole-number value from 16 to the maximum score of 48 is possible. </span>

(c) What is $P(X = 0)$?

<span style="color:#5858d0"> The probability that Gretchen never rolls a 1 is 0.233, from problem 1.  So the probability that she scores 0, which only happens when she rolls a 1, is 1 - 0.233 = 0.767.
...
<span style="color:#5858d0"> About 77% of the probability is at 0, since she rolls a 1 at some point 77% of the time.  The rest of the probability is spread out between the values 16 and 48, although the middle values around 30-35 are much more likely.  This weird shape means that the expected value of $X$ is around 8, which is actually not possible for Gretchen to score. </span>
